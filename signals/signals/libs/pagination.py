...
class CustomPagination(drf_pagination.LimitOffsetPagination):
    ...
    def paginate_queryset(self, queryset, request, view=None):
        """
        Overwrite https://github.com/tomchristie/django-rest-framework/blob/master/rest_framework/pagination.py#L347
    ...
    def get_paginated_response(self, data, extra_meta={}):
        if not self.count:
            self.count = len(data)
        document = self._make_response_document(data, extra_meta)
        return Response(document)

    def _make_response_document(self, data, extra_meta={}):
        meta = OrderedDict((
            ('request', self.request.build_absolute_uri(self.request.get_full_path())),
